...
rx = lambda x,y: x/r(x,y)
ry = lambda x,y: y/r(x,y)
u = lambda x,y: scipy.special.hankel1(0,1j*s*r(x,y))
ux = lambda x,y: -1j*s*scipy.special.hankel1(1,1j*s*r(x,y))*rx(x,y)
uy = lambda x,y: -1j*s*scipy.special.hankel1(1,1j*s*r(x,y))*ry(x,y)
...
phi = (scipy.sparse.linalg.spsolve(M,beta0.real)
       +1j*scipy.sparse.linalg.spsolve(M,beta0.imag))

phi = Q.dot(phi)
J = J(s)
jeta = np.dot(J,eta)
Lambda = (-0.5*eta + scipy.sparse.linalg.spsolve(M,jeta.real)
          +1j*scipy.sparse.linalg.spsolve(M,jeta.imag))
...
kpsi = np.dot(K,psi)
phi = -(-0.5*psi+scipy.sparse.linalg.spsolve(M,kpsi.real)
       +1j*scipy.sparse.linalg.spsolve(M,kpsi.imag))
phi = Q.dot(phi)
Lambda = (scipy.sparse.linalg.spsolve(M,beta1.real)
          +1j*scipy.sparse.linalg.spsolve(M,beta1.imag))
psi = Q.dot(psi)
uh = np.dot(D,psi)
